...
│   │   ├── view-as-switcher.tsx     ← admin-only "View as teammate" dropdown (added 2026-07-01)
...
- **Entry point**: a **"View as" dropdown in the header** (`components/layout/view-as-switcher.tsx`), visible only to **real admins** (keyed on `realRole`, so an impersonated rep-level session can't see it). Lists all workspace members (`GET /api/team/members`); click one → `POST /api/impersonation {userId}` → redirect to `/dashboard` + refresh. While viewing-as it shows "Viewing as X" + a "Back to my account" item.
