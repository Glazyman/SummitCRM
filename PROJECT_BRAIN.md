...
- **Not yet**: tag chips on pipeline cards / leads table (Open Item #8).
...
| 8 | **Tag chips on pipeline cards & lead list** | Low | The tags UI is now live in the **lead side panel** (`TagPicker` — add/reuse/create custom tags; 2026-06-03). Remaining: surface the assigned tags as chips on pipeline kanban cards and in the leads table so buyer type is visible at a glance without opening a lead. (`LeadRow.tags` field already exists.) Consider seeding starter tags + a `Buyer: <name>` convention. |
