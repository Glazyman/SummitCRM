...
### Session 2026-06-01 (lead-status % + mini-chart on analytics + sized pie)
...
- **Mini bar chart** (21st.dev "mini-chart" look — custom div bars, hover highlight + neighbour dim, animated): `components/dashboard/daily-calls-mini-chart.tsx`. **Lives on the Analytics overview tab** (below the summary cards; was briefly tested on the dashboard, now removed from there). Plots **unique leads called per day** (`leads_called` = DISTINCT lead per UTC day) instead of raw calls, to match the per-person framing of the rest of analytics. Fed by `GET /api/analytics/calls-7d`, which returns both `calls` and `leads_called` per day for the last 7 days (rep → own activity, admin → workspace; JS bucketing, low-volume — could hit the 1000-row cap on a busy workspace). NOTE: the legacy `get_time_series_analytics` RPC is **email**-only (`sent/opened/...`), no call data — that's why a dedicated endpoint was needed.
- **Call Summary donut → "sized pie"** (21st.dev "sized pie chart" look). `SizedPie` in `analytics-client.tsx`: each call outcome is its own **concentric ring** whose arc length = its share of total calls and whose radius grows per segment (smallest value innermost). Each arc sits on a faint muted full-circle track; center keeps the `unique_leads` number + "leads called". Built with recharts (one `<Pie>` per ring via `flatMap`, single-element data, `startAngle=90`/`endAngle=90−sweep`, `cornerRadius=3`). Chart tooltip dropped (the breakdown list beside it already shows value + %).

---

*Last updated: 2026-06-01 — covers all sessions through 2026-06-01 (Activities → Tasks rename; gh-API commit workflow; mobile pass; untimed follow-ups + conflict greying + origin-context profile nav; rep permissions + Tags column removal; dashboard Tasks widget; rep-performance Today-bounce fix; batches moved to Import page; rep dashboard KPI cards; interest→pipeline removal; admin dashboard KPI cards; mobile header + drawer polish; mobile header dropdowns centered; analytics + team mobile layout; analytics per-person/all-calls toggle; pipeline Needs Buyer card; lead-status %; mini-chart moved to analytics (unique leads/day); Call Summary sized pie)*
