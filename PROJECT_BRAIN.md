...
24. **A donut/ring center label must share a `relative` box with ONLY the chart — never the chart + legend/breakdown.** The overlay pattern is `<div className="relative"><ResponsiveContainer/><div className="absolute inset-0 flex … justify-center">{number}</div></div>`. If the `relative` wrapper also contains the legend/outcome-breakdown siblings, `inset-0` spans chart + legend, so the flex-centered number is centered over the *combined* (taller) area and floats **above** the donut hole. Bit us on the analytics donut (2026-06-03) and again on both dashboard `CallDonut`s + the personal `FollowUpRing`/`CallDonut` in `my-activity.tsx` (2026-07-01) — the latter is what shows in a rep's "dash" (and in admin *View as*). Fix: wrap only the chart + overlay in the `relative` box and move the legend outside it; the correctly-built `FollowUpRing` was the reference. Also add `leading-none` to the big number so its line box doesn't add vertical drift.
