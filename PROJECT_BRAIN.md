...
No DB changes, no new API routes. Key files: `app/(dashboard)/call-mode/page.tsx`, `call-mode-client.tsx`, sidebar link in `components/layout/sidebar.tsx`. Reuses `components/leads/lead-full-panel.tsx`.
...
| 12 | **Call Mode session logging** | Medium | Requested 2026-06-11. Today Call Mode logs individual calls only (`call_logs`); there's no per-session record (who/when/preset/batch/queue size/calls logged/outcome breakdown/skipped/duration). Needs a `call_sessions` table + migration, create-on-start / finalize-on-end API, client wiring, and a view (rep history and/or admin oversight). Not started — scope with the user (what to capture, who can view). |
