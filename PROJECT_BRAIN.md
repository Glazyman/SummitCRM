...
- **Open in Word** (viewer footer, `doc`/`docx` only): opens the doc in **Word for the web** (Microsoft Office viewer `https://view.officeapps.live.com/op/view.aspx?src=<signedUrl>`) in a **new tab**. Was originally the `ms-word:` desktop protocol but on macOS that fell through to the default `.docx` app (LibreOffice), so switched to the web viewer per user. Hands Office the short-lived **signed URL** (`GET /api/documents/[id]`), not the cookie-protected `/raw`. The new tab is opened synchronously then redirected (popup-blocker dodge). NOTE: this sends the doc to **Microsoft's servers** (third party) — viewer is read-only; no PDF support.
