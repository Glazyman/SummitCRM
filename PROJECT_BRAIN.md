...
15b. **`git log -- <path>` (path-scoped history) hangs in this repo — even natively-ish.** Diagnosed 2026-06-02: plain `git status`, `git log` (no path), `git diff HEAD~1 HEAD`, `git show --stat` are all FAST and healthy; ONLY path-limited history simplification (`git log -- <path>`, `git log --follow`, `git fsck`) hangs, spinning in a `ll_diff_tree_paths`/`emit_path` recursion (confirmed via `sample <pid>`). Repo is on local APFS (NOT the cause — though `~/Desktop/SummitCRM` *is* inside macOS "Desktop & Documents in iCloud", that only matters for the agent env). No locks, no hooks, no fsmonitor, no gc lock. Fixes, low→high risk: (1) `git commit-graph write --reachable` (ran clean, additive), (2) `git repack -ad` (ran clean), (3) capped `timeout 120 git fsck --no-dangling` to name a bad object, (4) fresh `git clone` + `rsync -a --exclude=.git` the working tree in. Workaround meanwhile: use `git show --stat <sha>` / `git diff --stat A B` instead of `git log -- <path>`. Two unrelated landmines found: **git-lfs is globally `required=true` but not installed** (`git config --global filter.lfs.required false` or `brew install git-lfs`), and global `user.name`/`user.email` are still placeholders. NOTE: inside the Claude Code agent env, `git repack`'d pack mmap then failed with `fatal: mmap failed: Operation canceled` — that ECANCELED is the AGENT environment intercepting git syscalls, not the user's native Terminal; verify any git fix in the native Terminal.
...
| 4 | **Diagnosed the `git log -- <path>` hang** (see §12 quirk 15b): isolated to path-scoped history walking; ran `commit-graph write` + `repack -ad`; documented native-Terminal verification + git-lfs/identity landmines. No app code change. | — |

---

*Last updated: 2026-06-02 — covers all sessions through 2026-06-02 (analytics sized-pie reverted to donut; dashboard rep-performance switched to Today/7d/30d/All-time presets; admin-only pipeline rep/batch/date filters; git path-scoped-log hang diagnosed). Earlier: 2026-06-01 (Activities → Tasks rename; gh-API commit workflow; mobile pass; untimed follow-ups + conflict greying + origin-context profile nav; rep permissions + Tags column removal; dashboard Tasks widget; rep-performance Today-bounce fix; batches moved to Import page; rep dashboard KPI cards; interest→pipeline removal; admin dashboard KPI cards; mobile header + drawer polish; mobile header dropdowns centered; analytics + team mobile layout; analytics per-person/all-calls toggle; pipeline Needs Buyer card; lead-status %; mini-chart moved to analytics (unique leads/day); Call Summary sized pie; recharts 3.8 chart-type gotcha)*
