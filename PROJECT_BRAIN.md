...
| PDF→Word | pdfjs-dist (text extract) + docx 9.7.1 (Word build) | In-house PDF→`.docx` text conversion (`lib/documents/pdf-to-docx.ts`) for "Open PDF in Word". `serverExternalPackages:['pdfjs-dist']`; **needs Node 22** (`engines.node:"22.x"`) + `Promise.withResolvers` polyfill — see quirk 20. |
...
20. **pdfjs-dist v5 needs Node 22 — Vercel defaulting to Node 20 silently broke PDF→Word conversion (it just hung/500'd).** pdfjs 5.x calls `Promise.withResolvers`, which only exists on Node 22+. Two fixes applied together: (a) `engines.node: "22.x"` in package.json so Vercel uses Node 22; (b) a `Promise.withResolvers` polyfill at the top of `lib/documents/pdf-to-docx.ts` (belt-and-suspenders). Also `serverExternalPackages: ['pdfjs-dist']` in next.config so it loads from node_modules at runtime. The conversion logic itself is correct (prototyped successfully: 161 clean lines from a 4-page PDF). **Caveat for testing in THIS agent sandbox**: pdfjs `getDocument` (and any pdf-parse) reliably **hangs** here due to the sandbox's documented Node CPU-starvation — so PDF conversion can't be re-run locally; verify on Vercel. Helper has a 45s `withTimeout` + scanned-PDF guard so the route never hangs forever (returns 422 with a clean message instead).
...
| — | **Couldn't runtime-test conversion in the agent sandbox** — pdfjs (and pdf-parse) hang here due to sandbox Node CPU-starvation; the logic was proven earlier (161 lines from a real PDF) and `next build` is green. Verify on Vercel. | — |
