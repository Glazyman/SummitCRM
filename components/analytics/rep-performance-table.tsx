...
const AVATAR_BG = ['bg-primary','bg-primary','bg-primary','bg-primary','bg-primary','bg-primary']
              ...
              {!loading && sorted.map((rep, i) => {
                ...
                return (
                  <tr key={rep.user_id} className={cn(
                    ...
                    isTop ? 'bg-secondary hover:bg-secondary' : 'hover:bg-muted/30',
                  )}>
                    <td className="px-4 py-3 text-muted-foreground font-medium">
                      {isTop ? <Trophy className="h-4 w-4 text-foreground" /> : i + 1}
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div className={cn('flex h-8 w-8 items-center justify-center rounded-full text-primary-foreground text-xs font-semibold shrink-0', avatarBg(rep.user_id))}>
                    ...
                    <td className="px-4 py-3">
                      <span className={cn('font-semibold', rep.open_rate >= 28 ? 'text-foreground' : rep.open_rate >= 18 ? '' : 'text-foreground')}>
                        {rep.open_rate}%
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <span className={cn('font-semibold', rep.reply_rate >= 5 ? 'text-foreground' : rep.reply_rate >= 3 ? '' : 'text-foreground')}>
                        {rep.reply_rate}%
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <span className={cn('font-semibold', rep.bounce_rate > 3 ? 'text-foreground' : rep.bounce_rate > 1.5 ? 'text-foreground' : '')}>
                        {rep.bounce_rate}%
                      </span>
                    </td>
